...
SQLite database, one process lock, and at most one ETHUSDT position acquired by
this worker.  Exchange/account ETH balances are never used to infer position
...
import eth_testnet_learning_store as learning_store
...
POLICY_CONFIG_PATH = ROOT / "config" / ("binance-testnet-eth-hourly-policy.json" if HOURLY_MODE else "binance-testnet-eth-active-policy.json")
...
def _load_policy_config(path: Path = POLICY_CONFIG_PATH) -> dict[str, object]:
    ...
    expected_policy = "eth_hourly_momentum_24h_t002_testnet_v1" if HOURLY_MODE else "eth_daily_momentum_30d_t10_testnet_v1"
    ...
    if not isinstance(order, dict) or order.get("symbol") != "ETHUSDT" or order.get(
        "mode"
    ) != "long_cash":
...
def _signal_in_context(market_data_client: object) -> dict[str, object]:
    ...
    return {
        ...
        "close_30d": old_close,
        "momentum": momentum,
        "feature_schema": "eth_15m_causal_v1" if INTERVAL == '15m' else "eth_hourly_causal_v1" if HOURLY_MODE else "eth_daily_causal_v1",
        "features": features,
...
def _recover_timestamp_halt(
    ...
    with _process_lock(_account_lock_path(lock_path)):
        with closing(_connect(db_path)) as db:
            ...
            if open_orders:
                raise WorkerHalt(
                    "Clock-skew recovery refuses existing ETHUSDT open orders."
                )
    ...
    *,
    ...
    with closing(_connect(db_path)) as db:
        ...
        if open_orders:
            ...
            reason = (
                "Untracked ETHUSDT open order(s) detected; worker halted: "
                + ", ".join(identifiers)
            )
            _halt(db, reason)
            return {"action": "halted", "reason": reason}
        ...
        if HOURLY_MODE and os.getenv("BINANCE_TESTNET_AUTO_MODEL") == "true" and not exit_only:
            if last is None or int(last) < int(signal["candle_close_ms"]):
                import eth_hourly_testnet_learning
                import eth_hourly_model_authority
                try:
                    eth_hourly_testnet_learning.refresh(market)
                    signal = eth_hourly_model_authority.apply(signal)
                except Exception:
...
def _public_learning_status(db_path: Path | str) -> dict[str, object]:
    """Expose the active cadence learner without changing governance logic.

    Hourly/15m execution deliberately does not write into the immutable daily
    label contract.  Keep that daily evidence available, but make the proxy
    learner that is actually retrained on every new 15m label visible in the
    operator status response.
    """

    daily = _learning_status_nonfatal(db_path)
    if not HOURLY_MODE:
        return daily
    result = dict(daily)
    result["counter_scope"] = "daily_governance_evidence_not_active_cadence_training"
    result["active_cadence"] = INTERVAL
    status_path = STATE_DIR / "eth-testnet15m-learning-status.json"
    try:
        active = json.loads(status_path.read_text(encoding="utf-8"))
        if not isinstance(active, dict):
            raise ValueError("Active learner status must be an object.")
        result["active_cadence_learning"] = active
    except Exception as exc:
        result["active_cadence_learning"] = {
            "status": "unavailable",
            "error": f"{type(exc).__name__}: {exc}",
        }

    # The JSON file describes the last refresh attempt and can contain only an
    # error during a transport outage. Read current committed evidence directly
    # instead of turning missing fields into zero or recycling a stale snapshot.
    result.update(
        active_metrics_source="learner_database",
        active_metrics_available=False,
        active_metrics_observed_ms=None,
        active_metrics_error=None,
        active_label_counts=None,
        active_total_labels=None,
        active_model_id=None,
        active_last_training=None,
        active_last_label_end_ms=None,
        active_forward_scored_predictions=None,
    )
    try:
        with closing(_connect_read_only(
            STATE_DIR / "eth-testnet15m-learning.sqlite3"
        )) as db:
            # All counters belong to one committed learner snapshot, even if a
            # new label/model is persisted while this status request is running.
            db.execute("BEGIN")
            try:
                counts = dict(db.execute(
                    "SELECT origin, COUNT(*) FROM samples "
                    "WHERE y IS NOT NULL GROUP BY origin"
                ))
                latest = db.execute(
                    "SELECT id, created FROM models ORDER BY id DESC LIMIT 1"
                ).fetchone()
                last_label = db.execute(
                    "SELECT MAX(label_end) FROM samples WHERE y IS NOT NULL"
                ).fetchone()[0]
                forward = db.execute(
                    "SELECT COUNT(*) FROM predictions p JOIN samples s ON s.ts=p.ts "
                    "WHERE s.y IS NOT NULL AND p.created*1000 < s.label_end"
                ).fetchone()[0]
                observed_ms = _now_ms()
            finally:
                if db.in_transaction:
                    db.execute("ROLLBACK")
        result.update(
            active_metrics_available=True,
            active_metrics_observed_ms=observed_ms,
            active_label_counts=counts,
            active_total_labels=sum(counts.values()),
            active_model_id=latest["id"] if latest else None,
            active_last_training=latest["created"] if latest else None,
            active_last_label_end_ms=last_label,
            active_forward_scored_predictions=forward,
        )
    except Exception as exc:
        result["active_metrics_error"] = f"{type(exc).__name__}: {exc}"
    return result
...
def _unavailable_status_snapshot(
    ...
    return {
        ...
        "learning": _public_learning_status(db_path),
...
def status_snapshot(
    ...
    if not Path(db_path).exists():
        result = _unavailable_status_snapshot(
            db_path, lock_path, FileNotFoundError('ETH Testnet ledger has not started')
        )
        result.update(status='not_started', status_available=True, status_error=None,
                      execution_state_known=True, running=False, desired_running=False,
                      halted=False, position='cash')
        result['learning'] = {
            **result['learning'], 'status': 'not_started', 'last_error': None,
            'proposal_ready_for_review': False,
        }
        return result
    try:
    ...
    *,
    ...
    if command == "reset":
        ...
        with _process_lock(account_lock_path):
            ...
            if open_orders:
                raise WorkerHalt(
                    "Testnet epoch reset refuses existing ETHUSDT open orders."
                )
