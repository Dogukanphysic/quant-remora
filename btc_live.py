...
class Client:
    ...
    def market(self):
        ...
        return dict(bid=str(bid),ask=str(ask),filters=symbol['filters'],
                    ...
                    bollinger_touch=bollinger_touch(rows) if INTERVAL=='15m' else None)
...
def recover_authorization(db, client, *, connection=False):
    ...
    side = 'SELL' if s['phase']=='long' else 'BUY'
    intent = size(s,market,side,rates[side])
    if intent is None or client.test_order(intent)!={}:
        raise ValueError('Fresh test-order permission check failed')
    if first!=snapshot():
    ...
    with db:
        if read(db)!=s or db.execute('SELECT COUNT(*) FROM orders WHERE applied=0').fetchone()[0]:
            raise ValueError('Ledger changed during recovery')
        db.execute('CREATE TABLE IF NOT EXISTS authorization_recoveries(id INTEGER PRIMARY KEY,previous_state TEXT,evidence TEXT)')
        db.execute('INSERT INTO authorization_recoveries(previous_state,evidence) VALUES (?,?)',(previous,json.dumps(proof)))
        s.update(halted=None,stopped=True,last_authorization_recovery=proof)
        s.pop('halt_reason',None)
        write(db,s)
    return dict(ok=True,orders_submitted=0,worker_started=False,halt_cleared=True)
...
def tick(db,client):
    existing = read(db)
    if existing:
        stored_strategy = existing.get('strategy_mode','ema_atr')
        requested_strategy = BB_STRATEGY if BOLLINGER_TOUCH else 'ema_atr'
        if stored_strategy.startswith('bollinger_') and requested_strategy != BB_STRATEGY:
            raise ValueError('Restart with --bollinger-touch to preserve strategy mode')
        if stored_strategy != requested_strategy and (existing['pending'] or existing['halted']):
            raise ValueError('Cannot change strategy with pending order or halted ledger')
    ...
    if BOLLINGER_TOUCH and s.get('strategy_mode') != BB_STRATEGY:
        ...
        s['strategy_mode'] = BB_STRATEGY
        s['strategy_changed_at'] = market['now']
        s['last_bar'] = market['bar']  # First new candle is the first eligible touch.
        with db:
            db.execute('CREATE TABLE IF NOT EXISTS strategy_changes(id INTEGER PRIMARY KEY,ts REAL,previous_state TEXT,new_strategy TEXT)')
            db.execute('INSERT INTO strategy_changes(ts,previous_state,new_strategy) VALUES (?,?,?)',
                       (market['now'],previous,BB_STRATEGY))
            write(db,s)
...
def main():
    global MODEL_DECISIONS, USE_ALL_ALLOCATED_FUNDS, AUTO_ALLOCATE_SPOT, BOLLINGER_TOUCH
    ...
    parser.add_argument('--bollinger-touch',action='store_true')
    ...
    BOLLINGER_TOUCH = args.bollinger_touch
    ...
    if BOLLINGER_TOUCH and (args.interval!='15m' or args.action!='run' or MODEL_DECISIONS):
        raise ValueError('Bollinger touch requires run --interval 15m without --model-decisions')
